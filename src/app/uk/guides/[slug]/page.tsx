...
export function generateStaticParams() {
  return getGuidesByRegion("uk").map((g) => ({ slug: g.slug }));
}
...
export async function generateMetadata({
  ...
  if (!guide || guide.region !== "uk") return { title: "Not Found" };
  return {
...
export default async function UKGuidePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const guide = getGuideBySlug(slug);
  if (!guide || guide.region !== "uk") notFound();
      ...
      <section className="border-t-2 border-walnut bg-manila/30 py-12 md:py-16">
        <div className="mx-auto max-w-[800px] px-6 text-center md:px-12">
          <h2 className="font-heading text-2xl text-walnut">
            Generate your complete UK document pack
          </h2>
          <p className="mx-auto mt-3 max-w-lg font-body text-sm font-light text-graphite">
            All UK restaurant documents customised to your business. Ready in minutes.
          </p>
          <div className="mt-8">
            <RubberStampCTA href="/uk/generate/" size="large">
              Generate Your UK Documents — £49
            </RubberStampCTA>
          </div>
        </div>
      </section>
